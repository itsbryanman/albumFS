use albumfs::stego::{png::PngCodec, CarrierCodec, ChunkMeta, CodecError};
...
fn roundtrip_various_sizes() {
    ...
    let codec = PngCodec;
    let cap = codec.capacity_bytes(&make_carrier(dir.path(), 200, 200)).unwrap();

    for n in [0usize, 1, 64, (cap / 2) as usize, cap as usize] {
        let p = make_carrier(dir.path(), 200, 200);
        let mut payload = vec![0u8; n];
        rand::thread_rng().fill(&mut payload[..]);
        let meta = ChunkMeta { chunk_index: 7, flags: 1 };
        codec.write_chunk(&p, meta, &payload, &[]).unwrap();
        let (rm, rp) = codec.read_chunk(&p, &[]).unwrap();
...
fn payload_too_large_errors() {
    ...
    let err = codec
        .write_chunk(&p, ChunkMeta { chunk_index: 0, flags: 0 }, &payload, &[])
        .unwrap_err();
...
fn plain_image_is_not_a_carrier() {
    ...
    let p = make_carrier(dir.path(), 64, 64);
    let codec = PngCodec;
    let err = codec.read_chunk(&p, &[]).unwrap_err();
